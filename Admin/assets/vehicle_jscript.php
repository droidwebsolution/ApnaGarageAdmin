<?php 
    ...
    if (isset($_POST['vehicle_add'])) {
        ...
        $ccs = explode(',', $_POST['vh_cc']);
    
        if (count($modelNames) !== count($manufactureYears) || count($modelNames) !== count($ccs)) {
        ...
        for ($i = 0; $i < count($modelNames); $i++) {
            ...
            $ag_vehicle_cc = check_data($ccs[$i]);
            ...
            $codeCounter = intval(substr($maxCode, -2)) + $i + 1;
            $ag_vehicle_code = 'AGV_' . sprintf('%02d', $codeCounter);
        ...
        $msg = "Data Added Successfully for model";
